...
export function LocaleHomeClient({ locale }: { locale: Locale }) {
  ...
  return (
    <MobileLayout appShell className="app-screen gap-2 overflow-y-auto pb-[calc(env(safe-area-inset-bottom)+1rem)]">
      <TopSafeArea />
      <section className="animate-rise overflow-hidden rounded-[26px] border border-black/8 bg-[linear-gradient(160deg,#111111_0%,#050505_58%,#1b1b1b_100%)] px-4 py-3.5 text-white shadow-[0_18px_40px_rgba(17,17,17,0.14)]">
        ...
        <h1 className="mt-3 max-w-[8ch] text-[1.55rem] font-black leading-[0.9] tracking-[-0.06em] sm:text-6xl">
          {t("landing.heroTitle")}
        </h1>
        <p className="mt-2.5 max-w-[18rem] text-[11px] leading-4 text-white/68 sm:text-base">
          {t("landing.heroBody")}
        </p>

        {user ? (
          <div className="mt-3 rounded-[20px] border border-white/10 bg-white/10 p-3 backdrop-blur">
            ...
            <Link href="/dashboard" className="mt-2.5 block">
              <AccessibleButton className="min-h-[54px] w-full rounded-[18px] bg-accessible-yellow text-black hover:brightness-95">
                {t("landing.openMap")}
              </AccessibleButton>
            </Link>
          </div>
        ) : (
          <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold uppercase tracking-[0.16em] text-white/70">
      ...
      <section className="grid min-h-0 gap-2.5">
        <Link href={`/${locale}/auth/register?role=REQUESTER`} className="block">
          <div className="card-surface rounded-[22px] p-3 transition hover:-translate-y-0.5">
            <div className="flex items-start justify-between gap-3">
            ...
            <div className="mt-2.5 flex items-start gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[12px] bg-accessible-red/10 text-[1rem]">
                🆘
              </div>
              <div>
                <h2 className="text-[1.05rem] font-black leading-none tracking-tight">{t("auth.needHelpRole")}</h2>
                <p className="mt-1 text-[12px] leading-4 text-black/68">{t("landing.requesterCard")}</p>
              </div>
            </div>
          </div>
        </Link>
        <Link href={`/${locale}/auth/register?role=HELPER`} className="block">
          <div className="card-surface rounded-[22px] p-3 transition hover:-translate-y-0.5">
            ...
            <div className="mt-2.5 flex items-start gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[12px] bg-accessible-lime/16 text-[1rem]">
                🤝
              </div>
              <div>
                <h2 className="text-[1.05rem] font-black leading-none tracking-tight">{t("auth.wantHelpRole")}</h2>
                <p className="mt-1 text-[12px] leading-4 text-black/68">{t("landing.helperCard")}</p>
              </div>
            </div>
          </div>
        </Link>
      ...
      <section className="mt-auto grid gap-2.5 bg-[linear-gradient(180deg,rgba(244,239,232,0)_0%,rgba(244,239,232,0.92)_18%,rgba(238,246,241,0.98)_100%)] pb-[calc(env(safe-area-inset-bottom)+0.9rem)] pt-3">
        <Link href={`/${locale}/auth/login`}>
          <AccessibleButton className="min-h-[54px] w-full rounded-[18px] shadow-[0_14px_28px_rgba(17,17,17,0.12)]">
            {t("common.login")}
          </AccessibleButton>
        </Link>
        <Link href={`/${locale}/auth/register?role=REQUESTER`}>
          <AccessibleButton tone="secondary" className="min-h-[54px] w-full rounded-[18px] bg-white/96">
