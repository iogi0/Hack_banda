  ...
  return (
    <MobileLayout>
      ...
      <section className="card-surface rounded-[34px] p-5">
        ...
        {/* Role info pill */}
        <div className={`mt-3 rounded-[18px] border px-4 py-3 text-sm ${
          role === "REQUESTER"
            ? "border-accessible-red/20 bg-red-50 dark:bg-red-900/15 text-red-800 dark:text-red-300"
            : "border-accessible-lime/20 bg-green-50 dark:bg-green-900/15 text-green-800 dark:text-green-300"
        }`}>
          {role === "REQUESTER"
      ...
      <p className="pb-2 text-center text-sm text-[var(--openarm-muted)]">
        {t("auth.haveAccount")}{" "}
        <Link href={href("/auth/login")} className="font-bold text-[var(--openarm-text)] underline underline-offset-2">
          {t("common.login")}
        </Link>
      </p>
    </MobileLayout>
